...
BASE_URL = os.environ.get("REACT_APP_BACKEND_URL", "https://finance-book-hub.preview.emergentagent.com").rstrip("/")
API = f"{BASE_URL}/api"
...
def demo_token():
    r = requests.post(f"{API}/auth/login", json={"email": "demo@myfinancebook.in", "password": "Demo@123"})
...
# ---------- Allowlist ----------
class TestAllowlist:
    def test_allowlist_returns_three(self):
...
class TestLogin:
    ...
    def test_login_ca_success(self):
        r = requests.post(f"{API}/auth/login", json={"email": "ca@myfinancebook.in", "password": "CA@123456"})
        assert r.status_code == 200
        assert "token" in r.json()

    def test_login_non_allowlisted_returns_403(self):
        r = requests.post(f"{API}/auth/login", json={"email": "hacker@evil.com", "password": "whatever"})
        assert r.status_code == 403

    def test_login_wrong_password_returns_401(self):
        r = requests.post(f"{API}/auth/login", json={"email": "demo@myfinancebook.in", "password": "WrongPass"})
    ...
    def test_auth_me_with_token(self, auth_headers):
        ...
        # ensure _id / password are not leaked
        assert "_id" not in u
...
class TestPortfolio:
    def test_save_and_get_portfolio(self, auth_headers):
        holdings = [{"symbol": "TEST_RELIANCE", "qty": 5, "price": 2500.0},
                    {"symbol": "TEST_INFY", "qty": 10, "price": 1500.0}]
        watchlist = ["TEST_TCS", "TEST_HDFCBANK"]
        r = requests.post(f"{API}/portfolio/save", headers=auth_headers,
...
class TestAI:
    def test_ai_suggest_anthropic(self, auth_headers):
        r = requests.post(f"{API}/ai/suggest", headers=auth_headers,
    ...
    def test_ai_suggest_openai(self, auth_headers):
        r = requests.post(f"{API}/ai/suggest", headers=auth_headers,
